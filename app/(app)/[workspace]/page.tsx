...
export default async function DashboardPage({
  ...
  const [analytics, openLeads, activity, stages, companies, contacts] =
    await Promise.all([
      ...
      getLeads(workspace.id, { status: "open" }),
  ...
  return (
    <>
      <PageHeader
        title="Dashboard"
        subtitle={`Where ${workspace.name} stands today.`}
        actions={
          <>
            <NewCompanyDialog
      ...
      <div className="space-y-5 px-5 py-6 sm:px-8">
            ...
            <ul className="divide-y divide-line-soft">
              {activity.map((item) => {
                const href = item.lead
                  ? `/${workspace.slug}/leads/${item.lead.id}`
                  : item.contact
                    ? `/${workspace.slug}/contacts/${item.contact.id}`
                    : item.company
                      ? `/${workspace.slug}/companies/${item.company.id}`
                      : null;
                const context =
                  item.lead?.title ??
                  (item.contact
        ...
        {analytics.velocity.stalledCount > 0 && (
          <div className="card flex flex-wrap items-center justify-between gap-3 px-4 py-3">
            ...
            <Link href={`/${workspace.slug}/pipeline`} className="btn btn-ghost">
              Review pipeline
            </Link>
          </div>
        )}
